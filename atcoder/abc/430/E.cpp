...
int main() 
{
    ...
    while (t--) {
        string a, b; cin >> a >> b;
        a += string{a};
        peria::rolling_hash_powers<peria::mint::MOD, 5> powers{a.size()};
        peria::rolling_hash rh_a{a, powers};
        const int hash_b {peria::rolling_hash{b, powers}.hash()};
        const int n {static_cast<int>(b.size())};
        int ans {-1};
        for (int i{}; i+n-1<2*n; ++i) {
            const int l {i+1};
            const int r {i+n-1+1};
            if (rh_a.sub_hash(l, r, powers) == hash_b) {
                ans = r-n;
                break;
            }
        }
